...
class DataTableLoadTest1 extends TestCase
{
     /**
     * Define the environment-specific setup.
     */
    protected function setUp(): void
    {
        ...
        if (app()->environment('local')) {
            ...
            for ($i = 0; $i < 10000; $i++) {
                // $logDate = $faker->dateTimeBetween('2024-02-10', '2024-02-28')->format('Y-m-d');
                $logDate = $this->generateUniqueLogDate($users[$n],$i);
    
                DB::connection('epushserver')->table('devicelogs_2_2024')->insert([
                    'DownloadDate' => $faker->date,
                    'DeviceId' => $faker->randomNumber(),
                    'UserId' => $users[$n] ,
                    'LogDate' => $logDate,
                    'Direction' => 'in',
                    'AttDirection' => 'in',
